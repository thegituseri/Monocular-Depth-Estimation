...
class NYUDataset(Dataset):
    def __init__(self, zip_path, csv_path_within_zip, base_dir_within_zip, transform=None, max_rows=5945):
        ...
        self.base_dir = base_dir_within_zip
        ...
        # We'll open the zip file lazily (per worker)
        self.zip_file = None
    ...
    def __getitem__(self, idx):
        img_rel_path = self.data_info.iloc[idx, 0].strip()
...
def depth_loss(pred, target, image, lambda_smooth=0.1):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader):
            ...
            if i % 40 == 0:
                ...
                print(f" Epoch {i,j}, Loss: {loss.item():.4f}")
        j += 1
